本文的原作者是#link("https://alexander-huang.github.io")[Alex]，#strike[经授权后Ctrl
\+ C]

原文地址：#link("https://alexander-huang.github.io/2019/08/16/记一次manjaro变arch的奇妙经历")[传送门]

经本人测试：

文章内容的指令在Manjaro 18.1下没问题，最新测试版19.0
pre下，部分会报错，但是其他的关键的指令输入后目前本人没有发现问题，软件包也完全是Arch的。
...
<等等让我先解释一下是怎么发生的>
#quote(block: true)[
emmm，我在酷安上交流Linux使用的时候，因为用了Archlinux的话题，就被"Arch大邪教CN分教"的教职人员以及忠实信徒洗礼了一遍。便下定决心洗心革面，只要有空就入Archlinux。于是前晚上刻好了Archlinux的livecd，准备一气呵成，没想到居然被一个DNS的问题困住了（手机开无加密热点分享给电脑的，有老哥知道解决方法吗？），折腾30分钟无果，只好重启进入Manjaro。这时忽然想起了以前一位Antergos用户一日离奇变Archlinux（Antergos是arch的下游发行版），#strike[心想，这到底是人性的扭曲，还是道德的沦丧？但还没想出，]我就对manjaro动了手。
]

= 搞机过程
<搞机过程>
#strong[声明：以下方法并非正常操作，不保证不会出现问题，出现问题后可以在Google、archwiki等处寻找解决方案、或者询问各位大佬。因为使用以下方法导致损失请自行负责，本人不承担任何责任。使用者应具有基本的独立解决问题的能力！再次提醒：数据无价，请谨慎！]！
...
= 后记
<后记>
　　我使用的是manjaro18.04，安装后通过滚动更新到最新版本，不保证每个版本的manjaro都可以成功，也不保证此方法一直有效。即使你成功了，也有可能会在以后的某次滚动更新中滚挂。#strike[这不是arch的特性吗？]如果你们有更稳妥的方法，欢迎分享。

　　其实安装archlinux，用不着完全像ArchWiki教程那样一步一步完全自己配置。某位知乎网友提到，可以将配置比较类似的电脑上的archlinux通过备份还原软件复制到自己的电脑上，再经过一些很简单的配置流程即可。

　　不过，按照Arch
wiki安装系统，你可以更深入地了解Linux的系统构成，也可以在解决问题中找到普遍方法。#strike[貌似Arch用户们更倾向于把这看成是一种挑战，完成安装来证明自己有能力加入大邪～，如同原始部落规定让一定年龄的男孩去野外打一头狼来作为自己已经成年的标志？那我的这个行为该怎么定义？！]也许这正是arch的精神所在？好吧，等我下次有时间了再折腾。
...
#strong[最近更新于2020年1月30日21：00]，如果发现问题，欢迎在以下评论区或者酷安反馈。
