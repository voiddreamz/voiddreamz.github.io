...
但是除了大型的DE（Desktop Environment）之外，有着一些窗口管理器（Windows
Manager）,他们以轻便著名。
...
#link("https://i.loli.net/2020/02/07/8x9wjPT1aJ6uqC5.png")[i3wm]

#v(0.5em)
#line(length: 100%)
#v(0.5em)

#strong[注意：本文使用的是Fork自i3wm项目，进行添加和修改的i3-gaps，Arch的AUR中有此项目。其他的分发版想要使用，请自行编译，]#link("https://github.com/Airblader/i3")[i3-gaps]#strong[编译过程中的报错，大多原因为缺少依赖，可自行百度解决。]
...
此时你的表情一定是这样的：#link("https://i.loli.net/2020/02/21/MWZH62IiNo4DQlO.jpg")[i3-1.jpg]或是这样的#link("https://i.loli.net/2020/02/21/mwvpySHcrLFI389.gif")[i3-2.gif]。
...
= 重头戏：配置文件
<重头戏配置文件>
众所周知，软件的配置都放在一个配置文件。对于Linux，用户的配置文件都放在#sub[\/.config/中。i3wm的配置文件为]/.config/i3/config。

Mod+Enter键后进入终端，便可以用你喜欢的文本编辑器编辑i3wm。 \*\*\*
\#\#\# i3wm文件的简单配置
打开之后，可以看到文档的内容不算多，接下来分门别类地来谈谈其中的信息。
\#\#\#\#\# Mod键的设置 打开之后，在靠前的位置可以看到这么一行
...
默认使用Mod+Enter打开的终端 这个说不大准，在文档中有这么一行：
...
\*\*\* \#\#\#\# 其他的一些配置
此处推荐clone我的config，fork自#link("https://github.com/aeghn/prettyi3")[aeghn的prettyi3]，个人进行了一些简单优化。下文的一些配置都已经在我的config中改好了，进行一些简单的微调即可。此处放上链接：#link("https://github.com/voiddreamz/prettyi3")[传送门]
...
其中的PATH为壁纸文件的路径。 `--no-startup-id`为登录后自启动。
`--bg-fill`可以依照此篇#link("https://www.jianshu.com/p/a18df7a79058")[feh部分功能的使用]。
...
如果是要使用dunst，请将/prettyi3/light/config/dunst文件复制到#sub[\/.config/dunst，并且删掉]/.config/i3/config中的所有有关rofi的内容，#strong[并加入这么一行]：
...
#link("https://i.loli.net/2020/02/27/EQUDdWLTjcs6MuC.png")[light.png]
...
放上个知乎的链接吧，里面大佬也是挺多的：#link("https://www.zhihu.com/question/62251457/answer/196854381")[传送门]。
